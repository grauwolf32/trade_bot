...
def aboutpage():
    try:
        ...
        body = ""
        for inst in instruments:
            body += "instrument: %s  size: %f</br>\n"%(inst,info[inst]['size'])
            for date in dates[inst]:
                body += "date: %s start time: %s stop time: %s bid: %f ask: %f number of records: %d</br>\n" % \
                        (date,info[inst][date]['mintime'],info[inst][date]['maxtime'],info[inst][date]['avgbid'],\
                        info[inst][date]['avgask'],info[inst][date]['n_records'])

        html = "<html><head><title>%s</title><head><body>%s</body></html>" % ("Information",body)
        return html
    except Exception,e:
        return str(e)
